...
dopp_resolution = 12.5;              %  Hz  (there is minimal benefit in setting this <1/(4*Tcoh)
max_del_tau = 0.5;                               %  chips
max_del_doppler = 50; %1/Tcohs;                %  Hz
chip_center = 5;                                %  chips
dopp_center = 500;                           %  Hz
...
% True signal
true_amplitude = 0.9;
true_chip_delay = chip_center - 0.1;      %  chips
true_dopp_offset = dopp_center - 5;     %  Hz
true_phase = 0*pi/180;                          % rad
CN0dBHz = 35;                                      %  dB-Hz

% Spoofed signal
spoofed_amplitude = 0.9;
spoofed_chip_delay = chip_center + 0.1;                      %  chips
spoofed_dopp_offset = dopp_center + 5;         %  Hz
spoofed_phase = 45*pi/180;                                          % rad
prn_spoofed = 17;                                                          % works for any 1-32

% multipath signal4
multipath_amplitude = 0.3;
multipath_chip_delay = chip_center +0.2;                               %  chips
multipath_dopp_shift = dopp_center + 10;                                  %  Hz
multipath_phase = 90*pi/180;                      % rad

% Load PRN codes
load CA_code_table_50MHz
